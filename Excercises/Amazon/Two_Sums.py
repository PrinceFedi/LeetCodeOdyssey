...
class Solution:
    def maximumWealth(self, accounts: List[List[int]]) -> int:
        ...
        for account in accounts:
            if itertools.accumulate(account) > max:
                max = itertools.accumulate(account)
        return max
...
# Explanation
"""
For our solution, we take our given array and traverse through its indices. However, we don't want to give each index a
...
the elements in the original inputted array. Example: nums = [0, 2, 7, 15] 9
